...
/*
Package abcrpcclient implements a websocket-enabled Aero JSON-RPC client.
...
All notifications provided by abcd require registration to opt-in.  For example,
...
detected by type asserting the error in a *abcjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

  amount, err := client.GetBalance("")
  if err != nil {
  	if jerr, ok := err.(*abcjson.RPCError); ok {
  		switch jerr.Code {
  		case abcjson.ErrRPCUnimplemented:
