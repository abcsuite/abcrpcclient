...
// Client represents a Aero RPC client which allows easy access to the
// various RPC methods available on a Aero RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *abcjson.NotifyWinningTicketsCmd:
		c.ntfnState.notifyWinningTickets = true

	case *abcjson.NotifySpentAndMissedTicketsCmd:
		c.ntfnState.notifySpentAndMissedTickets = true

	case *abcjson.NotifyNewTicketsCmd:
		c.ntfnState.notifyNewTickets = true

	case *abcjson.NotifyStakeDifficultyCmd:
		c.ntfnState.notifyStakeDifficulty = true

	case *abcjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *abcjson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *abcjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled abcjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := abcjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := abcjson.MarshalCmd(id, cmd)
	if err != nil {
		return newFutureError(err)
	}
